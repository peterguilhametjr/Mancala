...
import java.util.*;
/**
 * CS151 Mancala Project Fall 2022 - BoardTwo
 * @author Buzz - Logan and Peter
 * @version 12/02/2022
 */

/**
 * Creates the second board if chosen from the frame class
 *
 */
public class BoardTwo extends MancalaBoard {
	...
	public BoardTwo(MancalaFrame frame) {
	...
	public void draw(Graphics2D g2) {
		...
		Ellipse2D.Double base = new Ellipse2D.Double(x, y - height / 2, width, height * 2);
		...
		g2.drawString("B1", x + width/8 + 5*xgap, y - 20);
		g2.drawString("B2", x + 2*width/8 + 5*xgap, y - 20);
		g2.drawString("B3", x + 3*width/8 + 5*xgap, y - 20);
		g2.drawString("B4", x + 4*width/8 + 5*xgap, y - 20);
		g2.drawString("B5", x + 5*width/8 + 5*xgap, y - 20);
		g2.drawString("B6", x + 6*width/8 + 5*xgap, y - 20);
		
		AffineTransform at = new AffineTransform();
        at.setToRotation(Math.toRadians(270), 100, 100);
        g2.setTransform(at);
        g2.drawString("Mancala B", -x + 100, y + 170);
        
        AffineTransform at2 = new AffineTransform();
        at2.setToRotation(Math.toRadians(90), 100, 100);
        g2.setTransform(at2);
        g2.drawString("Mancala A", 500, -5 * y + 100);
	}
